...
save_button.addEventListener('click', () => {
    ...
    if (!data)
        return alertToast(
            'error',
            'Cannot fetch data! Please refresh page. #0',
            false
        )
    ...
    fetch(`/api/guild/${data}`, {
        ...
        body: JSON.stringify({
            ...
            welcomer: {
                welcome: {
                    ...
                    channel: {
                        id:
                            welcome_channel[welcome_channel?.selectedIndex]
                                ?.value || '0',
                    },
                    // random_message: {
                ...
                goodbye: {
                    ...
                    channel: {
                        id:
                            goodbye_channel[goodbye_channel?.selectedIndex]
                                ?.value || '0',
                    },
                    // random_message: {
                    ...
                    // },
                    message:
                        goodbye_message?.value || 'Goodbye **{user.name}**',
                },
            },
        }),
        headers: {
        ...
        .then((json) => {
            console.log(json)
            if (json.status == 0)
                return alertToast('error', 'Failed to save data!', false)
            else if (json.status == 1)
                return alertToast('done', 'Saved guild settings.', true)

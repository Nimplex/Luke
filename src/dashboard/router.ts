...
export = (app: Application) => {
    app.use((req, res, next) => {
        if (req.session) req.session.logged = req.session.code && req.session.user && req.session.guilds
        next()
    })

    // Dashboard
    app.get('/', (req, res) => res.render('index', { logged: req.session?.logged, user: req.session?.user || [], guilds: Luke.guilds.cache.size }))
    app.get('/dashboard', (req, res) => {
        if (!req.session?.logged) return res.redirect(`https://discord.com/oauth2/authorize?client_id=${server.id}&redirect_uri=${process.argv[2] ? server.dev_uri : server.redirect_uri}&response_type=code&scope=identify%20guilds`)
        else {
    ...
    app.get('/dashboard/:id', (req, res) => res.redirect(`/dashboard/${req.params.id}/basic`))
    app.get('/dashboard/:id/:cat', async(req, res) => {
        ...
        cacheChannels.forEach((channel: Channel) => !['category', 'voice'].includes(channel.type) ? channels.push({ n: (channel as any).name, id: channel.id }) : null)

        const perms = new Permissions(guild.g.permissions)
        if (!perms.has(['MANAGE_GUILD', 'MANAGE_MESSAGES', 'VIEW_AUDIT_LOG'])) return res.redirect('/401')
        ...
        fetch('https://discord.com/api/oauth2/token', { method: 'POST', body: Data }).then(res => res.json()).then(json => {
            ...
            fetch('https://discord.com/api/users/@me', { method: 'GET', headers: { authorization: `${json.token_type} ${json.access_token}` } }).then(res => res.json()).then(user => {
                ...
                fetch('https://discord.com/api/users/@me/guilds', { method: 'GET', headers: { authorization: `${json.token_type} ${json.access_token}` } }).then(res => res.json()).then(guilds => {
                    ...
                    req.session!.guilds = []
                    guilds.forEach((guild: any) => (new Permissions(guild.permissions)).has(['MANAGE_GUILD', 'MANAGE_MESSAGES', 'VIEW_AUDIT_LOG']) ? req.session!.guilds.push({ g: guild, b: Luke.guilds.cache.get(guild.id) ? true : false }) : null)
    ...
    app.post('/api/guild/:id', async(req, res) => {
        ...
        if (!perms.has(['MANAGE_GUILD', 'MANAGE_MESSAGES', 'VIEW_AUDIT_LOG'])) return res.status(400).json({ status: 0, msg: 'Invalid permissions.' })
        ...
        await server.updateOne(Object.assign(server, {
